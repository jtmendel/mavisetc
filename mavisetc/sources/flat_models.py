...
class flat_source():
    ...
    def _set_filter(self, filt):
        ...
        if filter_num in mips_dict:
            td = np.trapz(((self.wavelength/mips_dict[filter_num])**(-2.))*ntrans/self.wavelength, self.wavelength)
            ntrans = ntrans/max(1e-70,td)

        if filter_num in spitzer_dict:
            td = np.trapz(((self.wavelength/spitzer_dict[filter_num])**(-1.0))*ntrans/self.wavelength, self.wavelength)
            ntrans = ntrans/max(1e-70,td)
